...
from telegram_checker.utils.helpers import copy_to_clipboard, parse_time_expression
...
def main():
    ...
    if args.skip_time:
        try:
            ...
            log.info(f"Error: {e}", EMOJI["error"])
            return
    ...
    path = Path(args.path)
    if not path.exists():
        log.info(f"Path does not exist: {path}", EMOJI["error"])
        return

    md_files = list(path.glob('*.md'))

    if not md_files:
        log.info(f"No .md files found in {path}", EMOJI["error"])
        return
